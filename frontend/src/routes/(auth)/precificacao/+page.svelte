<script lang="ts">
  ...
  let statusFilter = $state<'todos'|'prejuizo'|'abaixo'|'saudavel'>('todos')
  ...
  function calcRow(s: Servico, precoMap: Map<number, number | null>, matCost = 0): Row {
    ...
    const lucro = vendaAtual - direto * (1 + fixo)
    const margemPct = direto > 0 ? (lucro / (direto * (1 + fixo))) * 100 : 0
    let status: Row['status'] = 'SAUDÁVEL'
    if (vendaAtual < direto*(1+fixo) - 1e-6) status = 'PREJUÍZO'
    else if (margemPct < margem*100 - 1e-6) status = 'ABAIXO'
    return {
  ...
  function filteredRows(): Row[] {
    ...
    if (statusFilter !== 'todos') {
      const map: Record<typeof statusFilter, Row['status'] | null> = { todos: null, prejuizo: 'PREJUÍZO', abaixo: 'ABAIXO', saudavel: 'SAUDÁVEL' }
  ...
  function openConfirm(ids: number[]) { selectedIds = ids; showConfirm = true }
  function closeConfirm() { showConfirm = false; selectedIds = [] }
  async function confirmApply() { closeConfirm(); /* TODO: chama backend de atualização de preço */ }
            ...
            {#each filteredRows() as r}
              {#if !modoDetalhado}
                <tr class="border-t">
                  ...
                  <td class="text-right">{formatCurrency(r.custoDireto*(1+r.fixoFrac))}</td>
                  ...
                  <td class="text-center">
                    {#if r.status==='SAUDÁVEL'}
                      <Button variant="ghost" title="Visualizar detalhes">Ajustar preço</Button>
                    {:else if r.status==='ABAIXO'}
                      <Button variant="default" onclick={() => openConfirm([r.id])} title="Aplicar sugestão">Aplicar sugestão</Button>
                    {:else}
                      <Button variant="destructive" onclick={() => openConfirm([r.id])} title="Corrigir preço">Corrigir preço</Button>
                    {/if}
                  </td>
                </tr>
              {:else}
                <tr class="border-t">
                  ...
                  <td class="text-center">
                    <Button variant={r.status==='SAUDÁVEL'?'ghost':(r.status==='ABAIXO'?'default':'destructive')} onclick={() => openConfirm([r.id])}>
        ...
        <div class="mt-3 max-h-[220px] overflow-auto text-sm">
          <ul class="list-disc pl-5">
            {#each rows.filter(r => selectedIds.includes(r.id)) as r}
              <li>{r.nome}: {formatCurrency(r.vendaAtual)} → {formatCurrency(r.vendaSugerida)}</li>
            {/each}
          </ul>
        </div>
        <div class="mt-3 rounded-lg bg-green-50 p-3 text-green-800 text-sm">
          Impacto estimado no lucro mensal: {formatCurrency(rows.filter(r => selectedIds.includes(r.id)).reduce((acc, r) => acc + Math.max(0, r.vendaSugerida - r.vendaAtual), 0))}
        </div>
        <div class="mt-4 flex justify-end gap-2">
          <Button variant="secondary" onclick={closeConfirm}>Cancelar</Button>
          <Button onclick={confirmApply}>Confirmar ajuste</Button>
        </div>
      </div>
    </div>
  {/if}
